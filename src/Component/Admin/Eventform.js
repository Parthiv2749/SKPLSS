...
const EventForm = () => {
  ...
  useEffect(() => {
    if (Type === "Edit-Event" && Id && parseInt(Id) <= events.length) {
      setIsEditMode(true);
      const eventToEdit = events.find((e) => e.event_id === parseInt(Id));
      if (eventToEdit) {
        setFormData({
          title: eventToEdit.title,
          fromDate: eventToEdit.fromDate || eventToEdit.date || "",
          toDate: eventToEdit.toDate || eventToEdit.date || "",
          status: eventToEdit.status || "",
          category: eventToEdit.category || "",
          description: eventToEdit.description || "",
        });
        if (eventToEdit.image) {
          // For demonstration, you can show a preview from existing image URL
          fetch(eventToEdit.image)
            .then((res) => res.blob())
            .then((blob) =>
              setCoverImage(new File([blob], "cover.jpg", { type: blob.type }))
            );
        }
      }
    }
  ...
  useEffect(() => {
    ...
    const from = new Date(formData.fromDate).setHours(0, 0, 0, 0);
    const to = new Date(formData.toDate).setHours(0, 0, 0, 0);
